...
<head>
    ...
    <title>Doorprize Display — <?php echo e($event?->nama ?? 'Konvensi Improvement Dharma'); ?></title>
...
<header class="shrink-0 flex items-center justify-between px-8 py-3 border-b"
    ...
    <p class="text-xs uppercase tracking-widest" style="color:#7B91A1">
        <?php echo e($event?->nama ?? 'Konvensi Improvement Dharma'); ?>
...
<footer class="shrink-0 text-center py-2.5 border-t" style="border-color:rgba(64,100,126,0.2)">
    <p class="text-xs tracking-widest uppercase" style="color:rgba(123,145,161,0.3)">
        <?php echo e($event?->nama ?? 'Konvensi Improvement Dharma'); ?> · Dharma Group
    </p>
</footer>
...
function displayApp() {
    return {
        ...
        async poll() {
            ...
            try {
                const res  = await fetch('<?php echo e(route('doorprizes.displayStatus')); ?>?_=' + Date.now());
...
<?php /**PATH /var/www/AbsenKID/resources/views/admin/doorprizes/display.blade.php ENDPATH**/ ?>