...
<head>
    ...
    <title>Check-in Display — <?php echo e($event?->nama ?? 'Konvensi'); ?></title>
    ...
    <style>
        ...
        body {
            ...
            background: linear-gradient(rgba(243,244,246,0.92), rgba(243,244,246,0.92)),
                        url('<?php echo e($event->wallpaper_url); ?>') center/cover no-repeat fixed;
            <?php endif; ?>
        }
        @keyframes fadeInUp {
...
<body class="min-h-screen flex items-center justify-center text-gray-800 font-sans overflow-hidden" x-data="tvDisplay()">
    ...
    <div class="fixed top-0 left-0 right-0 flex items-center justify-between px-8 py-5 z-20">
        <img src="<?php echo e(asset('images/dharma-group.png')); ?>" class="h-16 w-auto object-contain" alt="Dharma Group">
        <img src="<?php echo e(asset('images/triputra-group1.png')); ?>" class="h-20 w-auto object-contain" alt="Triputra Group">
    </div>
    ...
    <div x-show="state === 'idle'" class="text-center px-8" x-cloak>
        <div class="float">
            <?php if($event?->logo): ?>
                <img src="<?php echo e(asset('storage/'.$event->logo)); ?>" class="h-36 w-auto object-contain mx-auto mb-8" style="filter: drop-shadow(0 8px 20px rgba(0,0,0,0.12))" alt="">
            <?php else: ?>
                <img src="<?php echo e(asset('images/dharma-group.png')); ?>" class="h-40 w-auto object-contain mx-auto mb-8" style="filter: drop-shadow(0 8px 20px rgba(0,0,0,0.12))" alt="">
            <?php endif; ?>
        </div>
        <h1 class="text-4xl font-extrabold text-gray-800 mb-3"><?php echo e($event?->nama ?? 'Konvensi Improvement Dharma'); ?></h1>
        <?php if($event?->tanggal): ?>
            <p class="text-gray-500 text-xl mb-2">📅 <?php echo e($event->tanggal->isoFormat('dddd, D MMMM Y')); ?></p>
        <?php endif; ?>
        <?php if($event?->lokasi): ?>
            <p class="text-gray-400 text-lg">📍 <?php echo e($event->lokasi); ?></p>
        <?php endif; ?>
    ...
    <div x-show="state === 'notif'" class="text-center px-8 w-full max-w-2xl" x-cloak>
        <div class="animate-in">
            ...
            <?php if($event?->logo): ?>
                <img src="<?php echo e(asset('storage/'.$event->logo)); ?>" class="h-44 w-auto object-contain mx-auto mb-4" style="filter: drop-shadow(0 8px 20px rgba(0,0,0,0.12))" alt="">
            <?php else: ?>
                <img src="<?php echo e(asset('images/dharma-group.png')); ?>" class="h-32 w-auto object-contain mx-auto mb-4" style="filter: drop-shadow(0 8px 20px rgba(0,0,0,0.12))" alt="">
            <?php endif; ?>
            <h2 class="text-3xl font-extrabold text-gray-800 mb-5"><?php echo e($event?->nama ?? 'Konvensi Improvement Dharma'); ?></h2>
...
<?php /**PATH /var/www/AbsenKID/resources/views/public/scan-tv.blade.php ENDPATH**/ ?>