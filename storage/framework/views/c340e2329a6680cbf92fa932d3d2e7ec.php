<?php $__env->startSection('title', 'Undangan'); ?>
<?php $__env->startSection('page-title', 'Manajemen Undangan'); ?>
<?php $__env->startSection('content'); ?>


<div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
    <div class="card p-4 text-center">
        <p class="text-2xl font-black text-green-500"><?php echo e($sentWa); ?></p>
        <p class="text-xs text-brand-slate mt-1">💬 WA Terkirim</p>
    </div>
    <div class="card p-4 text-center">
        <p class="text-2xl font-black text-brand-red"><?php echo e($failWa); ?></p>
        <p class="text-xs text-brand-slate mt-1">💬 WA Gagal</p>
    </div>
    <div class="card p-4 text-center">
        <p class="text-2xl font-black text-green-500"><?php echo e($sentEmail); ?></p>
        <p class="text-xs text-brand-slate mt-1">📧 Email Terkirim</p>
    </div>
    <div class="card p-4 text-center">
        <p class="text-2xl font-black text-brand-red"><?php echo e($failEmail); ?></p>
...
<div class="card mb-4 space-y-4">
    ...
    <div class="flex gap-3 items-center flex-wrap">
        <form method="POST" action="<?php echo e(route('admin.invitations.generate', $event)); ?>"
        ...
        <span class="text-sm text-brand-slate">Total undangan: <strong><?php echo e($invitations->total()); ?></strong></span>
        <a href="<?php echo e(route('admin.invitations.sendHistory')); ?>" class="btn-secondary ml-auto">📋 History Pengiriman</a>
    </div>
    ...
    <div class="border-t border-brand-slate/20 pt-4">
        ...
        <div class="flex gap-2 flex-wrap">
            <form method="POST" action="<?php echo e(route('admin.invitations.sendAll')); ?>"
        ...
        <p class="text-xs text-brand-slate mt-2">Hanya dikirim ke peserta yang belum berhasil menerima. Pastikan WA/Email diaktifkan di <a href="<?php echo e(route('admin.settings.index')); ?>" class="text-brand-navy underline">Pengaturan</a>.</p>
    </div>
    ...
    <div class="border-t border-brand-slate/20 pt-4">
        ...
        <div class="flex gap-2 flex-wrap">
            <form method="POST" action="<?php echo e(route('admin.invitations.confirmAll')); ?>"
    ...
    <?php if($subcoShortcuts->isNotEmpty()): ?>
    <div class="border-t border-brand-slate/20 pt-4">
        <p class="text-sm font-semibold text-brand-navy dark:text-white mb-3">⚡ Jalan Pintas — per SubCo</p>
        <div class="space-y-2">
            <?php $__currentLoopData = $subcoShortcuts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="flex items-center justify-between gap-3 p-2.5 rounded-lg bg-brand-cream/40 dark:bg-gray-700/30 flex-wrap">
                <div class="min-w-0">
                    <p class="text-sm font-medium truncate"><?php echo e($row->subco); ?></p>
                    <p class="text-xs text-brand-slate">
                        <?php echo e($row->belum_konfirmasi); ?> belum konfirmasi · <?php echo e($row->belum_hadir); ?> belum hadir
                    </p>
                </div>
                <div class="flex gap-2 shrink-0 flex-wrap">
                    <?php if($row->belum_konfirmasi > 0): ?>
                    <form method="POST" action="<?php echo e(route('admin.invitations.confirmAll')); ?>"
                          onsubmit="return confirm('Tandai semua peserta <?php echo e(addslashes($row->subco)); ?> yang belum konfirmasi sebagai \'Akan Hadir\'?')">
                        <?php echo csrf_field(); ?>
                        <input type="hidden" name="status" value="hadir">
                        <input type="hidden" name="subco" value="<?php echo e($row->subco); ?>">
                        <button class="text-xs px-3 py-1.5 rounded-lg font-medium text-white" style="background:#16a34a">✅ Konfirmasi Hadir</button>
                    </form>
                    <form method="POST" action="<?php echo e(route('admin.invitations.confirmAll')); ?>"
                          onsubmit="return confirm('Tandai semua peserta <?php echo e(addslashes($row->subco)); ?> yang belum konfirmasi sebagai \'Tidak Hadir\'?')">
                        <?php echo csrf_field(); ?>
                    ...
                    <?php endif; ?>
                    <?php if($row->belum_hadir > 0): ?>
                    <form method="POST" action="<?php echo e(route('admin.attendances.manualSubco')); ?>"
                          onsubmit="return confirm('Tandai HADIR LANGSUNG (tanpa scan) untuk semua peserta <?php echo e(addslashes($row->subco)); ?> yang belum hadir?\n\nIni akan langsung membuat catatan kehadiran fisik untuk <?php echo e($row->belum_hadir); ?> peserta.')">
                        <?php echo csrf_field(); ?>
                        <input type="hidden" name="subco" value="<?php echo e($row->subco); ?>">
...
<form method="GET" class="flex gap-2 mb-4 flex-wrap">
    <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari NPK atau Nama..." class="input flex-1 min-w-40">
    <input type="text" name="subco" value="<?php echo e(request('subco')); ?>" placeholder="Filter SubCo..." class="input w-40">
    <select name="filter" class="input">
        <option value="">Semua</option>
        <option value="belum_hadir" <?php echo e(request('filter')==='belum_hadir'?'selected':''); ?>>⏳ Belum Hadir</option>
        <option value="konfirmasi_hadir" <?php echo e(request('filter')==='konfirmasi_hadir'?'selected':''); ?>>📝 Konfirmasi Akan Hadir</option>
        <option value="konfirmasi_tidak_hadir" <?php echo e(request('filter')==='konfirmasi_tidak_hadir'?'selected':''); ?>>🚫 Konfirmasi Tidak Hadir</option>
        <option value="belum_konfirmasi" <?php echo e(request('filter')==='belum_konfirmasi'?'selected':''); ?>>❔ Belum Konfirmasi</option>
        <option value="no_wa"    <?php echo e(request('filter')==='no_wa'?'selected':''); ?>>❌ No. WA Kosong</option>
        <option value="no_email" <?php echo e(request('filter')==='no_email'?'selected':''); ?>>❌ Email Kosong</option>
    </select>
    <button class="btn-primary">Cari</button>
    <?php if(request('search') || request('filter') || request('subco')): ?>
        <a href="<?php echo e(route('admin.invitations.index')); ?>" class="btn-secondary">Reset</a>
    <?php endif; ?>
</form>
...
<div class="card overflow-hidden p-0">
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-brand-slate/10 dark:divide-gray-700">
                ...
                <tr class="hover:bg-brand-cream/50 dark:hover:bg-gray-700/30">
                    <td class="px-4 py-3 font-mono text-xs" style="color:#244C6B"><?php echo e($inv->employee->npk); ?></td>
                    <td class="px-4 py-3 font-medium"><?php echo e($inv->employee->nama); ?></td>
                    <td class="px-4 py-3"><span class="badge-blue"><?php echo e($inv->employee->subco); ?></span></td>
                    <?php $konfirmasi = $confirmations[$inv->employee_npk] ?? null; ?>
                    <td class="px-4 py-3 text-center">
                        ...
                        <?php elseif($konfirmasi === 'tidak_hadir'): ?>
                    ...
                    <td class="px-4 py-3 text-center">
                        <?php if($waSend?->status === 'sent'): ?>
                            <span class="inline-flex items-center gap-1 text-xs font-semibold text-green-600">✅ Terkirim</span>
                        <?php elseif($waSend?->status === 'failed'): ?>
                            <span class="inline-flex items-center gap-1 text-xs font-semibold text-brand-red" title="<?php echo e($waSend->error_message); ?>">❌ Gagal</span>
                        <?php elseif(!$inv->employee->no_telpon): ?>
                    ...
                    <td class="px-4 py-3 text-center">
                        <?php if($emailSend?->status === 'sent'): ?>
                            <span class="inline-flex items-center gap-1 text-xs font-semibold text-green-600">✅ Terkirim</span>
                        <?php elseif($emailSend?->status === 'failed'): ?>
                            <span class="inline-flex items-center gap-1 text-xs font-semibold text-brand-red" title="<?php echo e($emailSend->error_message); ?>">❌ Gagal</span>
                        <?php elseif(!$inv->employee->email): ?>
                            <span class="text-xs text-brand-slate">— kosong</span>
                        <?php else: ?>
                            <span class="text-xs text-brand-slate">Belum</span>
                        <?php endif; ?>
                    ...
                    <td class="px-4 py-3">
                        <div class="flex items-center justify-center gap-1">
                            <a href="<?php echo e(route('admin.invitations.qr', $inv)); ?>" target="_blank"
                               class="text-xs px-2 py-1 rounded text-brand-navy border border-brand-slate/30 hover:bg-brand-cream transition">QR</a>
                            <?php if(auth()->user()->hasRole('admin')): ?>
                            <form method="POST" action="<?php echo e(route('admin.invitations.sendOne', $inv)); ?>">
                                <?php echo csrf_field(); ?>
                                <button class="text-xs px-2 py-1 rounded text-white transition"
                                        style="background:#244C6B"
                                        onclick="return confirm('Kirim undangan ke <?php echo e(addslashes($inv->employee->nama)); ?>?')">
                                    📤 Kirim
                                </button>
                            </form>
                            <?php endif; ?>
    ...
    <div class="p-4 border-t border-brand-slate/10"><?php echo e($invitations->links()); ?></div>
</div>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/AbsenKID/resources/views/admin/invitations/index.blade.php ENDPATH**/ ?>