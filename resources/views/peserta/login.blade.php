...
<head>
    ...
    <title>Login Peserta — {{ $event?->nama ?? 'Konvensi Improvement Dharma' }}</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
</head>
<body class="min-h-screen flex items-center justify-center p-4 pt-20 font-sans"
      style="@if($event?->wallpaper_url) background: linear-gradient(135deg, rgba(26,46,64,0.75), rgba(26,56,80,0.75)), url('{{ $event->wallpaper_url }}') center/cover no-repeat; @else background: linear-gradient(135deg, #1a2e40 0%, var(--ev-primary) 50%, #1a3850 100%); @endif">
...
<div class="w-full max-w-md">
    ...
    <div class="bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 p-8 shadow-2xl">
        ...
        <form method="POST" action="{{ route('peserta.login.post') }}" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <div class="relative">
                    <select name="singkatan"
                        ...
                        @foreach($subcos as $s)
                            @if($s->singkatan)
                            <option value="{{ $s->singkatan }}" class="text-gray-900"
                                    {{ old('singkatan') == $s->singkatan ? 'selected' : '' }}>
                                {{ $s->singkatan }} — {{ $s->nama }}
                            </option>
                            @endif
                        @endforeach
                ...
                @if($subcos->where('singkatan', '!=', null)->isEmpty())
